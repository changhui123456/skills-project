...
from huaweicloudsdkas.v1 import AsClient, ListWarmPoolInstancesRequest
...
parser = argparse.ArgumentParser(description='查询暖池实例列表')
...
def main():
    ...
    try:
        ...
        for _ in range(MAX_PAGES):
            request = ListWarmPoolInstancesRequest()
            request.scaling_group_id = args.scaling_group_id
            request.limit = args.limit
            if marker:
                request.marker = marker

            response = client.list_warm_pool_instances(request)
        ...
        print(f"as.list_warm_pool_instances 查询失败: {e}")
        exit(1)
