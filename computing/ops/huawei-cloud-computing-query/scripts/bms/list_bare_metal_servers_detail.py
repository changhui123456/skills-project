...
from huaweicloudsdkbms.v1.model import ListBareMetalServersDetailRequest
...
# 参数
parser = argparse.ArgumentParser(description="查询裸金属服务器详情列表（OpenStack原生，无总数）")
...
parser.add_argument("--status", type=str, choices=["ACTIVE", "BUILD", "ERROR", "HARD_REBOOT", "REBOOT", "REBUILD", "SHUTOFF"], help="裸金属服务器状态")
...
try:
    ...
    # 构建请求
    request = ListBareMetalServersDetailRequest()
    if args.flavor:
    ...
    if has_custom_filter:
        ...
        for _ in range(MAX_PAGES):
            ...
            response = client.list_bare_metal_servers_detail(request)
        ...
        # 判断是否还有更多数据（此API无count，通过多查的第 FETCH_SIZE 条判断）
        ...
        # 渲染结果
        render(display_servers, has_more=has_more, next_marker=next_marker)
except Exception as e:
    print(f"bms.list_bare_metal_servers_detail 查询失败: {e}")
    exit(1)
