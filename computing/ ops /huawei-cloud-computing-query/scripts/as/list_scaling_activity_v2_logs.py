...
from huaweicloudsdkas.v1 import AsClient, ListScalingActivityV2LogsRequest
...
parser = argparse.ArgumentParser(description='查询弹性伸缩活动日志列表(v2)')
...
parser.add_argument('--type', type=str, default=None,
                    help='查询的伸缩活动类型（逗号分隔）：NORMAL-普通；MANNUAL_REMOVE-手动移除；MANNUAL_DELETE-手动移除并删除；MANNUAL_ADD-手动加入；ELB_CHECK_DELETE-ELB检查移除并删除；AUDIT_CHECK_DELETE-审计移除并删除；DIFF-期望实例数与实际不一致；MODIFY_ELB-LB迁移；ENTER_STANDBY-转入备用；EXIT_STANDBY-移出备用')
parser.add_argument('--status', type=str, default=None,
                    choices=['SUCCESS', 'FAIL', 'DOING'],
                    help='查询的伸缩活动状态：SUCCESS-成功；FAIL-失败；DOING-伸缩中')
...
def render(logs):
    if not logs:
        ...
        return
    header = "id\tstatus\ttype\tstart_time\tend_time\tscaling_value\tinstance_value\tdesire_value\tdescription"
    print(header)
    start = Offset
    end = min(Offset + 50, len(logs))
    for l in logs[start:end]:
        print(f"{getattr(l, 'id', '')}\t{getattr(l, 'status', '')}\t{getattr(l, 'type', '')}\t{getattr(l, 'start_time', '')}\t{getattr(l, 'end_time', '')}\t{getattr(l, 'scaling_value', 0)}\t{getattr(l, 'instance_value', 0)}\t{getattr(l, 'desire_value', 0)}\t{getattr(l, 'description', '')}")
    if len(logs) > end:
...
def main():
    ...
    try:
        ...
        for _ in range(MAX_PAGES):
            request = ListScalingActivityV2LogsRequest()
            ...
            response = client.list_scaling_activity_v2_logs(request)
        ...
        print(f"as.list_scaling_activity_v2_logs 查询失败: {e}")
        exit(1)
