...
Cloud Container Engine (CCE) is a high-reliability, high-performance enterprise-grade container management service that supports Kubernetes community native applications and tools. Kubernetes namespace is a logical grouping mechanism in Kubernetes for resource isolation and organization, which can organize resources in a cluster (such as Pods, Services, Deployments, etc.) into different namespaces, achieving logical isolation and management of resources. By creating Kubernetes namespaces, you can provide independent resource spaces for different applications, teams, or environments, improving the flexibility and security of resource management. This best practice will introduce how to use Terraform to automatically deploy a Kubernetes namespace, including querying availability zones and instance flavors, as well as creating infrastructure such as VPC, subnet, Elastic IP, CCE cluster, node, and Kubernetes namespace.
...
- [Kubernetes Namespace Resource (kubernetes\_namespace)](https://registry.terraform.io/providers/hashicorp/kubernetes/latest/docs/resources/namespace)
...
### 3. Query Availability Zones Required for Namespace Resource Creation Through Data Source

Add the following script to the TF file (such as main.tf) to inform Terraform to perform a data source query, the query results are used to create namespace related resources:
...
# Get all availability zone information in the specified region (defaults to inheriting the region specified in the current provider block when region parameter is missing) for creating namespace related resources
...
# Create VPC resource in the specified region (defaults to inheriting the region specified in the current provider block when region parameter is missing) for providing network environment for namespace
...
# Create VPC subnet resource in the specified region (defaults to inheriting the region specified in the current provider block when region parameter is missing) for providing network environment for namespace
...
# Create Elastic IP resource in the specified region (defaults to inheriting the region specified in the current provider block when region parameter is missing) for providing public network access capability for namespace
...
- **authentication\_mode**: The authentication mode of the cluster, assigned by referencing input variable `authentication_mode`, default is "rbac" for role-based access control
- **delete\_all**: Whether to delete all resources on termination, assigned by referencing input variable `delete_all_resources_on_terminal`, default is "true" to delete all resources
...
# Get all instance flavor information that meets specific conditions in the specified region (defaults to inheriting the region specified in the current provider block when region parameter is missing) for creating namespace related resources
...
### 11. Create Kubernetes Namespace Resource

Add the following script to the TF file to inform Terraform to create Kubernetes namespace resources:
...
# Create Kubernetes namespace resource in the CCE cluster for organizing and managing Kubernetes resources
...
- **metadata**: Namespace metadata configuration block
  - **name**: The name of the namespace, assigned by referencing input variable `namespace_name`
- **depends\_on**: Explicit dependency relationship, ensuring the CCE node resource is created before the namespace

> Note: Kubernetes namespace is a logical grouping mechanism in Kubernetes, used to organize resources in a cluster into different namespaces. The creation of a namespace depends on the Kubernetes provider being able to successfully connect to the CCE cluster, so it is necessary to ensure that the cluster and nodes have been created.
...
1. Run `terraform init` to initialize the environment (this will download the Kubernetes provider)
2. Run `terraform plan` to view the resource creation plan
3. After confirming that the resource plan is correct, run `terraform apply` to start creating Kubernetes namespace
4. Run `terraform show` to view the created Kubernetes namespace
...
- [Best Practice Source Code Reference For CCE Kubernetes Namespace](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/cce/kubenetes/namespace)